#!/bin/bash
#
# Source from https://github.com/hyperledger/composer/blob/master/packages/composer-website/jekylldocs/prereqs-ubuntu.sh
# Source from https://hyperledger.github.io/composer/latest/prereqs-ubuntu.sh
#
...
LATESTTAG=$(wget -qO- https://api.github.com/repos/creationix/nvm/releases | grep -E 'tag_name' | grep -oP 'v[0-9]{1,3}\.[0-9]{1,3}\.[0-9]{1,3}\"' | grep -oP 'v[0-9]{1,3}\.[0-9]{1,3}\.[0-9]{1,3}' | head -n 1)
curl -o- "https://raw.githubusercontent.com/creationix/nvm/${LATESTTAG}/install.sh" | bash
...
LATESTTAG=$(wget -qO- https://api.github.com/repos/docker/compose/releases | grep -E 'tag_name' | grep -oP '[0-9]{1,3}\.[0-9]{1,3}\.[0-9]{1,3}\"' | grep -oP '[0-9]{1,3}\.[0-9]{1,3}\.[0-9]{1,3}' | head -n 1)
sudo curl -L "https://github.com/docker/compose/releases/download/${LATESTTAG}/docker-compose-$(uname -s)-$(uname -m)" \
    -o /usr/local/bin/docker-compose
sudo chmod +x /usr/local/bin/docker-compose

# Install python v2 if required
set +e
# ERROR: $(echo "./prereqs-ubuntu.sh: line 127: python: command not found" | grep -c 2.) = 1
